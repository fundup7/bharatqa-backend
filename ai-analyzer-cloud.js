    ...
    return new Promise((resolve, reject) => {
        ...
        ffmpeg.ffprobe(videoPath, (err, meta) => {
            ...
            times.forEach((t, i) => {
                ...
                ffmpeg(videoPath).seekInput(t).frames(1).output(out).size('360x640')
                    .on('end', () => {
                    ...
                    .on('error', () => { done++; if (done === times.length) resolve(frames.sort((a, b) => a.timestamp - b.timestamp)); })
                    .run();
            });
        });
    });
...
async function analyzeBugReport(bugId, videoUrl, deviceStats, bugDescription) {
    ...
    try {
        ...
        console.log(`✅ Downloaded: ${Math.round(fs.statSync(videoPath).size / 1024)}KB`);
